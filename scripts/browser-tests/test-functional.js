...
(async () => {
  ...
  for (const [from, to] of [["/about.html", "/about-us/"], ["/emergency-tyre-change.html", "/24-7-mobile-tyre-replacement/"], ["/what-mobile-fitters-check-before-changing-a-tyre-on-a-hill.html", "/what-mobile-fitters-check-before-changing-a-tyre-on-a-hill/"], ["/puncture-repairs-whats-actually-being-done-to-your-tyre.html", "/puncture-repairs-whats-actually-being-done-to-your-tyre/"], ["/the-best-tyres-for-your-ford-on-edinburghs-roads.html", "/the-best-tyres-for-your-ford-on-edinburghs-roads/"], ["/the-best-tyres-for-edinburgh-west-lothian-roads.html", "/the-best-tyres-for-edinburgh-west-lothian-roads/"], ["/best-mobile-tyre-fitter-west-lothian/", "/mobile-tyre-fitting-west-lothian.html"], ["/tyres-bathgate-guide", "/van-tyre-replacement-services/"]]) {
    const r = await req(from); t(r.status === 301 && r.loc === to, `301 ${from} -> ${to}`, [r.status, r.loc]);
  }
  for (const [from, to] of [["/mobile-tyre-fitting-whitburn/", "/mobile-tyre-fitting-whitburn.html"], ["/mobile-tyre-fitting-in-addiewell/", "/mobile-tyre-fitting-addiewell.html"], ["/privacy-policy", "/privacy-policy.html"]]) { const r = await req(from); t(r.status === 301 && r.loc === to, `historical WordPress URL ${from} -> 301 -> ${to}`, [r.status, r.loc]); }
