#!/usr/bin/env node
// Phase 4B Batch B1 regression tests — the 4 new blog posts:
...
// scripts/phase3b-old-url-test.js, scripts/phase4-test.js and
// scripts/phase4b-batchA-test.js, which already cover these pages
// generically (sitemap inclusion, canonical self-match, single H1, no
// duplicate titles/meta, valid JSON-LD, no orphan pages, redirect
// single-hop). This file adds Batch-B1-specific assertions: visible FAQs
// matching FAQ schema, no leftover content-brief fragments, no copied
// paragraphs between the four new pages, and working phone/WhatsApp links.
...
//
// Run: `node scripts/phase4b-batchB1-test.js` (after `npm run build`).
...
const REDIRECTS_BEFORE = 16; // count confirmed unchanged from Phase 4B Batch A
...
// Content-brief / templated-artifact fragments seen elsewhere in the
// WordPress export (Target Keyword:, Students will..., etc.) — none of
// these should ever appear in freshly-written content.
...
async function main() {
  ...
  console.log(`Phase 4B Batch B1 regression tests — server at ${BASE}\n`);
  ...
  // ---- 6. visible FAQs match FAQ schema (where present) ----
  ...
  for (const p of FAQ_PAGES) {
    ...
    const schemaAnswers = faqBlock.mainEntity.map((q) => q.acceptedAnswer.text);
    const visibleQuestions = [...html.matchAll(/<span class="sfr-faq__q">([^<]*)<\/span>/g)].map((m) => m[1]);
    const visibleAnswers = [...html.matchAll(/sfr-faq__panel-inner">\s*<p>([^<]*)<\/p>/g)].map((m) => m[1]);
    if (visibleQuestions.length === schemaQuestions.length && visibleQuestions.every((q, i) => q === schemaQuestions[i])) ok();
    else fail(`${p}: visible FAQ questions don't match schema`, `visible=${JSON.stringify(visibleQuestions)} schema=${JSON.stringify(schemaQuestions)}`);
    if (visibleAnswers.length === schemaAnswers.length && visibleAnswers.every((a, i) => a === schemaAnswers[i])) ok();
    else fail(`${p}: visible FAQ answers don't match schema`, `visible=${JSON.stringify(visibleAnswers)} schema=${JSON.stringify(schemaAnswers)}`);
  }
  // the other two pages should NOT carry an FAQPage schema without visible FAQs
  for (const p of NEW_URLS.filter((p) => !FAQ_PAGES.includes(p))) {
  ...
  // ---- 8. no copied paragraphs between the four new pages ----
  console.log("\n== No copied paragraphs between the four new pages ==");
  const paraSets = NEW_URLS.map((p) => {
    const body = pageData[p].body;
    const section = body.split('class="sfr-legal__body"')[1] || body;
    const paras = [...section.matchAll(/<p>([^<]{40,})<\/p>/g)].map((m) => m[1].trim());
    return { url: p, paras };
  });
  ...
  // ---- 9. phone and WhatsApp links present and correctly formed ----
  ...
  // ---- 10. no broken internal links (hrefs to real pages/assets) ----
  ...
  // ---- 11. no orphan pages: each new URL is linked from somewhere else on the site ----
  console.log("\n== No orphan pages ==");
  const allDistHtml = siteFiles.map((f) => fs.readFileSync(f, "utf8")).join("\n");
  for (const p of NEW_URLS) {
  ...
  // ---- 12. sitemap and page inventory remain 1:1 ----
  ...
  // ---- 13. no redirect changes (still 16, unchanged from Batch A) ----
  ...
  // ---- optional: axe-core mobile-viewport accessibility pass ----
...
async function runAxeIfAvailable() {
  ...
  const srv = http.createServer((req, res) => {
    const url = new URL(req.url, `http://127.0.0.1:8942`);
    let p = decodeURIComponent(url.pathname);
    const filePath = p.endsWith("/") ? path.join(DIST_DIR, p, "index.html") : path.join(DIST_DIR, p);
    fs.readFile(filePath, (err, data) => {
  ...
  for (const p of NEW_URLS) {
    ...
    await page.goto(`http://127.0.0.1:8942${p}`, { waitUntil: "load" });
