...
> 301s (including the historical location URLs), `/mobile-tyre-fitting` and `/mobile-tyre-fitting/` to `/mobile-tyre-fitting.html` (its only indexable URL, one hop), `.html` to pretty-URL 301s, `/index.html` to `/`, pretty-path serving, `www` to apex, HTTP to
