...
* Static site built from the approved WordPress audit: **66 sitemap pages** (59 on pretty paths) + a dedicated `404.html`.
...
* **Pretty paths** (59 pages, served from differently named or same-named `.html` files) and their canonicals are defined once in the CloudFront
...
* Sitemap: `site/sitemap.xml` lists the 66 indexable pages using canonical URLs.
...
- [ ] **Pending audit URLs (no page yet, they return 404 and are NOT redirected; each waits for a named owner fact).** `/our-tyre-range/`: the audit row says "Keep ... unchanged" but the audit-branch text is a rewrite; its stock/availability/pricing claims ("we carry a wide range of tyre sizes on our vans", premium/mid-range/budget, caravan/trailer sizes, "no hidden charges") and its visible "Last updated" date are unconfirmed. Seven audit articles (text on `origin/claude/sfr-motors-audit-4ovs2r`) wait for: `/asymmetric-and-directional-tyres-difference/`, `/premium-or-budget-which-tyres-keep-you-safer/`, `/pirelli-silent-tyres-uk/`, `/michelin-radial-tire-history-innovation/` and `/how-to-choose-the-best-tyres-for-my-car-expert-buying-guide/` - each points readers to "our tyre range / what we carry" (which tyres SFR stocks; how-to-choose also ranks brands as premium/mid-range); `/mobile-tyre-fitter-near-me-myths/` - claims mobile fitting cost is comparable to a garage, that fitters carry the same branded tyres, and that vans carry workshop-bay equipment; `/which-is-the-best-mobile-tyre-fitting-service-provider-in-the-uk/` - tells readers to look for "a real business address" while the site publishes none (the correct registered office is not supplied).
...
site/                     the website source (edit here)
  *.html                  66 pages + 404.html (identical header/footer blocks; footer holds the company disclosure and Cookie settings button)
